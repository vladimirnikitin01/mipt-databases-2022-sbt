...
Напомню, что SecurityProvider является ключом к безопасности Oak, предоставляя доступ к отдельным модулям безопасности и связанным с ними конфигурациям. Начиная с версии 1.3.7 Oak предоставляет две реализации SecurityProvider , подходящие для установки OSGi и не-OSGi соответственно.
Основной пакет будет устанавливать специальный компонент OSGi ( SecurityProviderRegistration , помеченный как «Apache Jackrabbit Oak SecurityProvider» ), который регистрирует SecurityProvider после успешного разрешения всех обязательных ссылок. Этот новый подход устраняет проблемы, связанные с первоначальной реализацией поставщика безопасности, и был перенесен в существующие ветки.

Интерфейс SecurityConfiguration определяет функциональные возможности, общие для всех модулей, связанных с безопасностью. Помимо простых параметров конфигурации, базовый интерфейс определяет средства для правильной инициализации данного модуля безопасности и предоставляет внутренний механизм Oak для обеспечения надлежащей проверки и согласованности данных для всех операций, связанных с безопасностью.

Вот такой пример, приложен на официальном сайте, настройка SecurityProvider.


![image](https://user-images.githubusercontent.com/58188954/166812433-3334730c-c536-40f5-85df-33955b62dc4e.png)
...
Этому выделена даже отдельная [страница](https://jackrabbit.apache.org/jcr/jackrabbit-team.html) на сайте.

На сайте написано, что Jackrabbit является проектом Apache Software Foundation, а также перечислено множество коммитеров (Id,Name, Organization).

![image](https://user-images.githubusercontent.com/58188954/166813901-1c6825f4-2200-41d8-8dab-8dea7ad8b2a6.png)

Почетные и бывшие члены.

![image](https://user-images.githubusercontent.com/58188954/166813987-4e34fac2-55ee-4dad-b231-770ab8bd86bc.png)

И люди, которые внесли свой вклад в этот проект посредством предложений, исправлений или документации.
![image](https://user-images.githubusercontent.com/58188954/166814242-fe54a9a5-990b-4c8c-8ae3-f4c58fd96dd8.png)


**p)Создайте свои собственные данные для демонстрации работы СУБД.**

В итоге использовал код Python и онлайн генератор английских фамилий я сгенерировал свою [базу данных](https://github.com/vladimirnikitin01/mipt-databases-2022-sbt/blob/hw_4/demobase_Nikitin.txt), где у нас есть пара (фамилия человека) и соответствующая ему строчка из 20 символов



**q)Как продолжить самостоятельное изучение языка запросов с помощью демобазы. Если демобазы нет, то создайте ее.**
Можно читать документацию и смотреть видео(литературу перечислил выше), также поможет изучение языка Java, он упростит понимание и написание сложных команд. Демобазу возьмем из пункта **p**.

**r)Где найти документацию и пройти обучение**
- [официальный сайт](https://jackrabbit.apache.org/jcr/index.html)
- [Канал Jackrabbit Technologies](https://www.youtube.com/user/jackrabbittech)

**s)Как быть в курсе происходящего**

Для начала есть [официальный сайт](https://jackrabbit.apache.org/jcr/index.html), на котором регулярно отображаются изменения, также есть их [github](https://github.com/apache/jackrabbit), где тоже можно смотреть на актуальные изменения. Но, к сожалению, нет никаких других источников информации.
